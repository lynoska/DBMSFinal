...
Responders are able to insert, update, and delete each component of an incident directly through the database. On the web interface, however, responders may only add and query incidents. Refer to the img directory for tables with stored data and the sql directory for [creation](sql/tables.sql) and [insertion/update](sql/cases.sql) of tables.
